...
func TestSimplePolicyEngineDefaultAllow(t *testing.T) {
	...
	if !res.Allowed {
		t.Fatalf("expected allow by default, got %v", res)
	}
}
...
func TestSimplePolicyEngineExplicitAllow(t *testing.T) {
	...
	if !res.Allowed {
		t.Fatalf("expected allow by rule, got %v", res)
	}
}
...
func TestSimplePolicyEngineExplicitDeny(t *testing.T) {
	...
	if res.Allowed {
		t.Fatalf("expected deny by rule")
	}
	if res.Policy != "deny-admin" {
		t.Fatalf("expected policy deny-admin, got %s", res.Policy)
	}
}
...
func TestSimplePolicyEngineHeaderMatch(t *testing.T) {
	...
	if !res.Allowed {
		t.Fatalf("expected allow by header rule, got %v", res)
	}
}
