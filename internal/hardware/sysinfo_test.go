...
func TestDetectSystemInfoMissingProc(t *testing.T) {
	info := detectSystemInfo("linux", func(string) ([]byte, error) { return nil, errors.New("no /proc") })
	if info.MemoryKnown || info.MemoryGB() != 0 || info.NumCPU < 1 {
		t.Fatalf("expected unknown memory without panic, got %+v", info)
	}
	for _, goos := range []string{"darwin", "windows"} {
		if info := detectSystemInfo(goos, fakeReader(nil)); info.MemoryKnown {
			t.Fatalf("%s: memory must be reported unknown, got %+v", goos, info)
		}
	}
}
